...
2. **`app/build.gradle.kts`** — target around 80 lines, per the plan's Build Configuration section. Key points:
   ...
   - `BuildConfig` fields injected at build time: `A0PROD_HOST` (default `"j.your-tailnet.ts.net"`), `GIT_SHA` (short, via `providers.exec` if available; otherwise `"unknown"`), `BUILD_DATE` (ISO date)
